...
export default function TaskInput({
  ...
  return (
    <div className="input-box flex h-[65px] w-full justify-start overflow-hidden border border-transparent bg-[rgb(246,246,246)] focus-within:border-[#b83f45] focus-within:shadow-[0_2px_5px_rgba(246,37,37,0.3),0_2px_10px_rgba(246,37,37,0.3)]">
      <label
        className={clsx(
          "relative m-0 h-[65px] w-[45px] shrink-0 cursor-pointer border border-transparent bg-transparent p-0",
          !isListEmpty && "invisible",
        )}
      >
        <input
          type="checkbox"
        ...
        <span
          aria-hidden
          className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rotate-90 transform text-[22px] leading-none transition-colors duration-[150ms] ${
            isAllSelected ? "text-[#b83f45]" : "text-[#bfbfbf]"
          }`}
        >
          ❯
        </span>
